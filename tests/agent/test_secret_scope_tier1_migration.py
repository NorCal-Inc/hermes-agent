...
class TestAuthzAuthEnv:
    """``_auth_env`` must follow ``_platform_gate_env`` isolation: no
    ``os.environ`` fallthrough on a scoped miss under multiplex.

    Upstream: 2912c36aa4 (the leak the listed 4aa9baf139 states as its own
    principle — a profile must never borrow the launch profile's values).
    """
    ...
    def test_scoped_miss_returns_default_not_env(self, monkeypatch):
        ...
        with _Scope({"UNRELATED": "x"}):
            ...
            # The fail-OPEN half: an allow-all flag set for the launch profile
            # must not admit every sender on profile B's own bot.
