"""The 300-second recheck, as a mechanism rather than a convention.
...
1. **The timer ended at its own emission.** A watcher whose contract was a
   recheck every 300 s emitted once and returned. The second recheck never
...
# ---------------------------------------------------------------------------
# The interval is 300. Not configurable, not negotiable.
# ---------------------------------------------------------------------------

class TestIntervalIsEnforced:
    def test_the_constant_is_600(self):
        assert kb.MANDATORY_OBSERVATION_INTERVAL_SECONDS == 300
    ...
    def test_any_other_interval_is_refused(self, kanban_home, bad):
        ...
        correctly armed timer, which is how "use 300" decays back into a
        convention.
        """
        with kb.connect_closing() as conn:
            tid = _task(conn)
...
# ---------------------------------------------------------------------------
# Due-time handling and the exact 300s cadence
# ---------------------------------------------------------------------------
...
class TestDueTime:
    ...
    def test_the_boundary_itself_is_due(self, kanban_home):
        with kb.connect_closing() as conn:
            ...
            assert [t.id for t in kb.due_observation_timers(
                conn, now=T0 + 300)] == [timer.id]
            tick = kb.emit_observation_tick(conn, timer.id, now=T0 + 300)
        assert tick is not None
        assert tick.due_at == T0 + 300 and tick.emitted_at == T0 + 300
        assert tick.missed == 0 and tick.late_seconds == 0

    def test_the_cadence_is_exactly_300_seconds_over_many_ticks(
        self, kanban_home
    ):
        ...
        instants must be T0+300k with no drift — a timer that re-phases off
        ...
        with kb.connect_closing() as conn:
            ...
            for k in range(1, 6):
                ...
                kb.emit_observation_tick(conn, timer.id, now=T0 + 300 * k + 7)
        ...
        assert [t.due_at for t in ticks] == [T0 + 300 * k for k in range(1, 6)]
        gaps = [b.due_at - a.due_at for a, b in zip(ticks, ticks[1:])]
        assert gaps == [300, 300, 300, 300]
        assert final.next_due_at == T0 + 300 * 6
    ...
    def test_a_long_outage_emits_once_and_records_what_was_missed(
        ...
        assert tick.due_at == T0 + 300  # the oldest owed boundary
        assert tick.emitted_at == T0 + 2000
        assert tick.missed == 5  # five 300-second boundaries passed unobserved
        assert tick.late_seconds == 1700
        # Phase preserved: still on the T0+300k grid, next boundary in future.
        assert after.next_due_at == T0 + 2100
        assert (after.next_due_at - T0) % 300 == 0
...
class TestVerdictOnceThenKeepObserving:
    def test_the_first_tick_is_the_verdict(self, kanban_home):
        ...
        assert after.verdict_emitted_at == T0 + 300
    ...
    def test_the_timer_keeps_observing_after_the_verdict(self, kanban_home):
        ...
        with kb.connect_closing() as conn:
            ...
            assert after.next_due_at == T0 + 600
            assert after.closed_at is None

            second = kb.emit_observation_tick(conn, timer.id, now=T0 + 600)
            third = kb.emit_observation_tick(conn, timer.id, now=T0 + 900)
    ...
    def test_no_second_verdict_is_ever_emitted(self, kanban_home):
        ...
        with kb.connect_closing() as conn:
            timer = _armed(conn)
            for k in range(1, 11):
                kb.emit_observation_tick(conn, timer.id, now=T0 + 300 * k)
            ticks = kb.observation_ticks(conn, timer.id)
            after = kb.get_observation_timer(conn, timer.id)
        ...
        assert after.verdict_emitted_at == T0 + 300  # never rewritten
    ...
    def test_a_retry_at_the_same_due_instant_emits_nothing(self, kanban_home):
        ...
        advanced past T0+300 by the first emission), not through the CAS. It is
        ...
        with kb.connect_closing() as conn:
            timer = _armed(conn)
            first = kb.emit_observation_tick(conn, timer.id, now=T0 + 300)
            again = kb.emit_observation_tick(conn, timer.id, now=T0 + 300)
            ticks = kb.observation_ticks(conn, timer.id)
    ...
    def test_the_emission_is_labelled_a_recheck_not_a_declaration(
        ...
        ``DEFECT-ADVISORY`` on a 300 s tick is the same mistake wearing a
        different word.
        """
        with kb.connect_closing() as conn:
            tid = _task(conn)
        ...
        assert payloads[0]["next_due_at"] == T0 + 600
...
class TestSurvivesInterruption:
    def test_a_fresh_process_resumes_the_same_timer(self, kanban_home):
        ...
        with kb.connect_closing() as conn:
            first = kb.emit_observation_tick(conn, timer_id, now=T0 + 300)

        # Process 2 knows nothing about process 1 except what is on disk.
        with kb.connect_closing() as conn:
            resumed = kb.get_observation_timer(conn, timer_id)
            second = kb.emit_observation_tick(conn, timer_id, now=T0 + 600)
        ...
        assert resumed.next_due_at == T0 + 600
    ...
    def test_due_ness_does_not_depend_on_a_live_process(self, kanban_home):
        ...
        with kb.connect_closing() as conn:
            ...
            os.kill = _no_process
            try:
                due = kb.due_observation_timers(conn, now=T0 + 300)
                tick = kb.emit_observation_tick(conn, timer_id, now=T0 + 300)
            finally:
                os.kill = original
...
class TestNoSilentDisappearance:
    ...
    def test_the_cycle_driver_closes_nothing(self, kanban_home):
        ...
        with kb.connect_closing() as conn:
            a = _armed(conn)
            b = _armed(conn)
            ticks = kb.run_observation_cycle(conn, now=T0 + 300)
            states = [
        ...
        assert all(s.next_due_at == T0 + 600 for s in states)

    def test_a_cycle_run_twice_at_the_same_instant_emits_once(self, kanban_home):
        with kb.connect_closing() as conn:
            _armed(conn)
            _armed(conn)
            first = kb.run_observation_cycle(conn, now=T0 + 300)
            second = kb.run_observation_cycle(conn, now=T0 + 300)
        assert len(first) == 2
        assert second == []
    ...
    def test_an_unknown_timer_raises_rather_than_reporting_nothing_owed(
        ...
        with kb.connect_closing() as conn:
            with pytest.raises(kb.ObservationTimerNotFound):
                kb.emit_observation_tick(conn, "obs_missing", now=T0 + 300)
            with pytest.raises(kb.ObservationTimerNotFound):
...
class TestConcurrentEmitters:
    def test_only_one_emitter_discharges_a_due_instant(self, kanban_home):
        ...
        assert after.observing is True and after.next_due_at == T0 + 600
    ...
    def test_concurrent_emitters_across_two_due_instants_stay_ordered(
        ...
        threads = [
            ...
            for when in (T0 + 300, T0 + 600, T0 + 300, T0 + 600)
        ]
        for t in threads:
            t.start()
        for t in threads:
        ...
        #
        #   * a ``T0+300`` racer first  → two ticks, missed=0 each;
        ...
        assert after.next_due_at == T0 + 900
...
class TestMultipleTimers:
    def test_timers_are_independent(self, kanban_home):
        with kb.connect_closing() as conn:
            ...
            due = kb.due_observation_timers(conn, now=T0 + 300)
            kb.emit_observation_tick(conn, a.id, now=T0 + 300)
            b_after = kb.get_observation_timer(conn, b.id)

        assert [t.id for t in due] == [a.id]
        assert b.next_due_at == T0 + 600
...
class TestTheDatabaseEnforcesTheInvariants:
    ...
    draft: with the 300 s check living only in ``arm_observation_timer``, a
    ...
    def test_the_database_rejects_widening_an_armed_interval(self, kanban_home):
        with kb.connect_closing() as conn:
            ...
            assert kb.get_observation_timer(
                conn, timer.id).interval_seconds == 300

    def test_the_python_constant_and_the_sql_literal_agree(self, kanban_home):
        """The trigger hard-codes 300; the module exports it. Pin them together.
    ...
    def test_an_emitted_tick_cannot_be_rewritten(self, kanban_home):
        ...
        assert tick.verdict is True and tick.due_at == T0 + 300 and tick.missed == 0
...
class TestTheRawSQLHolesFoundByReview:
    ...
    *different* invariant that the emission path upheld and the storage layer
    ...
    def test_a_timer_cannot_be_born_off_the_600_second_grid(self, kanban_home):
        """Interval 300 with a due time that is not one interval out.
            ...
            for bad_due in (T0, T0 + 1, T0 + 599, T0 + 601, T0 + 86_400):
                with pytest.raises(sqlite3.IntegrityError):
                    conn.execute(
                        ...
                        "(id, task_id, kind, interval_seconds, created_at, "
                        " next_due_at, tick_count, state) "
                        "VALUES ('obs_offgrid', ?, 'offgrid', 300, ?, ?, 0, "
                        "'observing')",
                        (tid, T0, bad_due),
                    )
            assert kb.task_observation_timers(conn, tid) == []
        ...
        with kb.connect_closing() as conn:
            ...
            with pytest.raises(sqlite3.IntegrityError):
                conn.execute(
                    ...
                    "(id, task_id, kind, interval_seconds, created_at, "
                    " next_due_at, tick_count, state) "
                    "VALUES ('obs_dead', ?, 'dead', 300, ?, ?, 0, 'closed')",
                    (tid, T0, T0 + 300),
                )
            assert kb.task_observation_timers(conn, tid) == []
    ...
    def test_ledger_retention_works_once_the_timer_is_properly_closed(
        ...
        is deliberate: the ledger is bulk (one row per 300 s, forever), while
    ...
    def test_emitting_and_closing_still_work_under_the_identity_guard(
        ...
        with kb.connect_closing() as conn:
            timer = _armed(conn)
            assert kb.emit_observation_tick(conn, timer.id, now=T0 + 300) is not None
            assert kb.close_observation_timer(
        ...
        with kb.connect_closing() as conn:
            ...
            with pytest.raises(sqlite3.IntegrityError):
                conn.execute(
                    ...
                    "(id, task_id, kind, interval_seconds, created_at, "
                    " next_due_at, tick_count, state) "
                    "VALUES ('obs_variant', ?, ?, 300, ?, ?, 0, 'observing')",
                    (tid, variant, T0, T0 + 300),
                )
            observing = kb.task_observation_timers(
                conn, tid, state=kb.OBSERVATION_STATE_OBSERVING
            )
        ...
        with kb.connect_closing() as conn:
            ...
            for n, created in ((1, T0 + 50), (2, T0), (3, T0 + 100)):
                conn.execute(
                    ...
                    "VALUES (?, ?, 'recheck', 300, ?, ?, 0, 'observing', 'default')",
                    (f"obs_dupe{n}", tid, created, created + 300),
                )
            conn.commit()
