...
def _start_process(
    ...
    ``stdin`` is closed for the same class of reason. A supervised executor is
    non-interactive by construction and this module never writes to its stdin
    (``_drain`` calls ``communicate()`` with no input). Left unset, the child
    *inherits* the controller's stdin, and an agent CLI that reads trailing
    stdin then blocks until a descriptor nobody owns reaches EOF. ``codex
    exec`` does exactly this ("Reading additional input from stdin..."), so an
    inherited never-EOF stdin turns every independent verification into a
    silent full-timeout burn with zero bytes on both streams — indistinguishable
    from a hung model, and fatal precisely because it looks like one.
    """
    kwargs: dict = {
        "cwd": cwd,
        "env": env,
