...
def build_webhook_parser(subparsers, *, cmd_webhook: Callable) -> None:
    ...
    wh_sub.add_argument(
        "--preset",
        default="",
        help="Apply a preset subscription template (lifewiki-nightly, "
        "stripe-payment-success, stripe-payment-failure, "
        "stripe-checkout-abandonment, stripe-subscription-cancellation, "
        "stripe-dispute-opened, stripe-refund-issued)",
    )
    wh_sub.add_argument(
        "--deliver-only",
        action="store_true",
